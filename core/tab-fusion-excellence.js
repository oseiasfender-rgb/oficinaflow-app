...
function renderAdminShellHTML(){
  const nav=officialNavigation();
  return '<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>OficinaOS V551.33 — Administração</title><style>body{font-family:Arial,sans-serif;background:#f8f6f3;color:#22160c;margin:0;padding:24px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px}.card{background:white;border:1px solid #e7dacd;border-radius:16px;padding:16px;box-shadow:0 8px 20px rgba(0,0,0,.06)}button,input{padding:10px;border-radius:10px;border:1px solid #d8c8b8}button{background:#111827;color:white}pre{white-space:pre-wrap;background:#111827;color:#f8f6f3;padding:16px;border-radius:14px;overflow:auto;max-height:420px}</style></head><body><h1>OficinaOS — Administração V551.33</h1><p>Contas, Auditoria, Alertas, Busca Global e Configurações oficializadas.</p><div class="grid"><div class="card"><h2>Contas</h2><button onclick="show(AdminShell.contasResumo())">Abrir</button></div><div class="card"><h2>Auditoria</h2><button onclick="show(AdminShell.auditoria())">Abrir</button></div><div class="card"><h2>Alertas</h2><button onclick="show(AdminShell.gerarAlertas())">Gerar</button></div><div class="card"><h2>Busca Global</h2><input id="q" placeholder="cliente, placa, orçamento..."> <button onclick="show(AdminShell.buscaGlobal(document.getElementById(\\'q\\').value))">Buscar</button></div><div class="card"><h2>Configurações</h2><button onclick="show(AdminShell.configuracoesGet())">Abrir</button></div><div class="card"><h2>Auditoria de Fusão</h2><button onclick="show(TabFusionExcellence.audit())">Rodar</button></div></div><h2>Resultado</h2><pre id="out">Aguardando...</pre><script src="../core/storage-adapter.js"></script><script src="../core/clean-start.js"></script><script src="../core/admin-shell.js"></script><script src="../core/tab-fusion-excellence.js"></script><script>function show(x){document.getElementById("out").textContent=JSON.stringify(x,null,2)}</script></body></html>';
}
